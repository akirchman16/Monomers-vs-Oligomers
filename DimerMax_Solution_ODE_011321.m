...
% dimers and going back until equilibrium. This is not a stochastic model,
...
% maximum of trimers or tetramers or k-mers.
...
while ~Equilibrium_1 && ~Equilibrium_2
%     Tracks the number of iterations of the code
    Loops = Loops+1;
    i = Loops;
    
%     Euler Method to numerically solve coupled ODEs
    Matrix = [(1-k_l*L_1(i)*dt) (k_s*dt); (k_l*L_1(i)*dt) (1-k_s*dt)]; %matrix of 'changes'
    State_1 = [L_1(i); L_2(i)]; %matrix of current states
...
%     Testing for Equilibrium;
    if Loops > 100
        ...
        if (Avg_Change1 <= 1e-4) && (abs(diff(Limits1)) <= 1e-4)
            Equilibrium_1 = 1;
        end
        if (Avg_Change2 <= 1e-4) && (abs(diff(Limits2)) <= 1e-4)
            Equilibrium_2 = 1;
        end
    end
end
...
ylim([min(min(L_1),min(L_2)) max(max(L_1),max(L_2))]);
title('Monomers and Dimers in Solution');
legend('Monomers','Dimers');