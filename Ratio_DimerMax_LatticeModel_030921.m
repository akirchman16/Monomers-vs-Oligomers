...
InitialVelocity = zeros(1,numel(Percent_Monomer));
Y = zeros(numel(Percent_Monomer),1001);

X = 0:1.5/1000:1.5; %x-values for linear fit of initial rate
...
%     yline(Equilibrium_Coverage(Loops),'k',['\rho = ', num2str(Percent_Monomer(Loops))]);
    ...
    InitialVelocity(Loops) = FracCover(Loops,10)/t(Loops,10);  %average linear rate of change for first 10 events
    Y(Loops,:) = InitialVelocity(Loops)*X; %y-values for linear fit of initial reaction velocity
    figure(1);
    plot(X,Y(Loops,:),':k','LineWidth',0.25,'HandleVisibility','off');
end
